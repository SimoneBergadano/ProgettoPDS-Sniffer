...
use crate::{Data, L4Protocol, string_from_icmpcode, string_from_mac, Value, Filter};
...
fn packet_analyzer(p: Packet, data: &Arc<Mutex<Data>>, filter: Filter){
    ...
    if p.data[12]==0x08 && p.data[13]==0x00 && (filter == Filter::All || filter == Filter::IpV4Only){
    ...
    if p.data[12]==0x86 && p.data[13]==0xDD && (filter == Filter::All || filter == Filter::IpV6Only){
    ...
    if p.data[12]==0x08 && p.data[13]==0x06 && (filter == Filter::All || filter == Filter::ArpOnly){
...
pub fn analyzer_job(mut cap: Capture<Active>, data: &Arc<Mutex<Data>>, verbose_mode: bool, filter: Filter){
    ...
    loop{
        ...
        match res{
            Ok(packet) => { packet_analyzer(packet, &data, filter); }
            Err(e) => {
                match e {
